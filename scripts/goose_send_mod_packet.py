...
for p in packets:
    ...
    if gooseTest(p):
        ...
        # Append the datSet value to a list for selection
        datSetList.append(str(gd[datSet]))

###############################
# Have user select the packet to modify
###############################
print('Select Goose packet to modify and send.')
for e in range(len(datSetList)):
    print('[%d] %s'%(e,datSetList[e]))
pnum = input('Select number:')
pnum = int(pnum)
if DEBUG: print('User Selected: %d'%(pnum))
#sys.exit()
...
print('\n\n###############')
print('ENCODED Goose Data: %s'%(en_gd))
print('###############')
...
print('Resend this packet (time will be updated)?')
send_p = input('Send [y/N]:')
if send_p in ['Y','y']: print('Not Implemented Yet, But Nice Try!!!')
if DEBUG: print('User Selected: %d'%(send_p))
